...
def user_display_name(session: Session, user: User) -> str:
    ...
    person = None
    if user.email:
        person = session.scalar(select(Personnel).where(Personnel.email == user.email).order_by(Personnel.updated_at.desc()))
    if not person and user.name:
        person = session.scalar(select(Personnel).where(Personnel.name == user.name).order_by(Personnel.updated_at.desc()))
