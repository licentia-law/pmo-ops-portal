...
function ByProjectPanel({ rows }: { rows: any[] }) {
  return <SidePanel title="프로젝트별 이력 건수"><div style={{ overflowX: "hidden" }}><table className="pmo-table" style={{ fontSize: 14, width: "100%", tableLayout: "fixed" }}><colgroup><col style={{ width: 50 }} /><col /><col style={{ width: 80 }} /></colgroup><thead><tr><th style={{ textAlign: "center", fontSize: 14 }}>순위</th><th style={{ textAlign: "center", fontSize: 14 }}>사업명</th><th style={{ textAlign: "center", fontSize: 14 }}>이력 건수</th></tr></thead><tbody>{rows.map((r) => <tr key={r.code}><td className="num" style={{ textAlign: "center", color: "var(--tx-3)", fontWeight: 600, fontSize: 14 }}>{r.code === "etc" ? "—" : r.rank}</td><td style={{ textAlign: "center", color: "var(--tx-3)", fontWeight: 600, fontSize: 14, whiteSpace: "normal", wordBreak: "keep-all", overflowWrap: "anywhere", lineHeight: 1.45 }}>{r.name}</td><td className="num" style={{ textAlign: "center", color: "var(--tx-3)", fontWeight: 600, fontSize: 14 }}>{r.count.toLocaleString()}</td></tr>)}</tbody></table></div></SidePanel>;
}
