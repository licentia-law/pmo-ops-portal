  ...
  return <section className="pmo-panel" style={{ padding: "20px 22px 14px" }}>
    ...
    <div style={{ overflowX: "auto", marginLeft: -22, marginRight: -22 }}>
      <table className="pmo-table pmo-table--recent">
        ...
        <tbody>{logs.map((r) => { const slashIdx = String(r.author ?? "").indexOf("/"); const isSplit = slashIdx > -1; const authorHead = isSplit ? String(r.author).slice(0, slashIdx) : String(r.author ?? ""); const authorTail = isSplit ? String(r.author).slice(slashIdx + 1) : ""; return <tr key={r.id}><td style={{ textAlign: "center" }}><button className="pmo-btn" style={{ width: 24, minWidth: 24, height: 24, padding: 0, justifyContent: "center", fontSize: 12 }} onClick={() => onEdit(r)} title="편집" aria-label="진행 이력 편집">✏</button></td><td className="num" style={{ color: "var(--tx-3)", fontWeight: 600, fontSize: 14, whiteSpace: "nowrap" }}>{String(r.datetime ?? "").replace(" ", "\u00A0\u00A0\u00A0")}</td><td style={{ color: "var(--tx-3)", whiteSpace: "pre-wrap", wordBreak: "break-word", lineHeight: 1.6, fontWeight: 600, fontSize: 14 }}>{r.summary}</td><td style={{ color: "var(--tx-3)", fontWeight: 600, fontSize: 14, whiteSpace: "normal", wordBreak: "keep-all", lineHeight: 1.6 }}>{isSplit ? <><span>{authorHead}/</span><br /><span>{authorTail}</span></> : <span>{authorHead}</span>}</td><td style={{ textAlign: "center" }}><LogStateChip label={r.stateLabel} /></td></tr>; })}</tbody>
      </table>
    </div>
  </section>;
}
